...
@Controller
    ...
    public ResponseEntity<Integer> countMessages(){
                return ResponseEntity.ok(contacts.size());
    ...
    public ResponseEntity<Void> addMessage(@RequestBody String text, @PathVariable("index") Integer index) {
        contacts.add(index, "NEW MESSAGE");
        return ResponseEntity.accepted().build();
...
*/
    ...
    public ResponseEntity<Void> addContact(@RequestBody Contact contact) {
        ...
        return ResponseEntity.accepted().build();
    }// curl -X POST localhost:8080/contacts -H "Content-Type: application/json" -d "{\"name\":\"Bob\",\"number\":\"239239\"}"

    @DeleteMapping("contacts/{index}")
    public ResponseEntity<Void> deleteContact(@PathVariable("index") Integer index) {
        contacts.remove((int) index);
        return ResponseEntity.noContent().build();
    }// curl -X DELETE localhost:8080/contacts/0 -H "Content-Type: application/json"

    @GetMapping("contacts")
    public ResponseEntity<List<Contact>> getContacts(){
        return ResponseEntity.ok(contacts);
    }// curl -X GET localhost:8080/contacts -H "Content-Type: application/json"

    @PutMapping("contacts/{index}")
    public ResponseEntity<Void> updateContactParametres(@PathVariable("index") Integer i, @RequestBody Contact contact) {
        contacts.remove((int) i);
        contacts.add(i, contact);
        return ResponseEntity.accepted().build();
    }// curl -X PUT localhost:8080/contacts/0 -H "Content-Type: application/json" -d "{\"name\":\"Bob\",\"number\":\"239239\",\"email\":\"proger@gmail.com\"}"

    @GetMapping("contacts/{index}")
    public ResponseEntity<Contact> getContact(@PathVariable("index") Integer index) {
        return ResponseEntity.ok(contacts.get(index));
    }// curl -X GET localhost:8080/contacts/0 -H "Content-Type: application/json"
